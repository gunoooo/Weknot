...
class FeedAdapter(private val context: Context, private val feeds: List<Feed>) : Adapter<FeedViewHolder>() {
    ...
    private fun initView(holder: FeedViewHolder, position: Int) {
        holder.binding.writerName.text = feeds.get(position).writerName
        holder.binding.content.text = feeds.get(position).feedComent

        Glide.with(context).load(feeds.get(position).writerPicture).into(holder.binding.profileImage)
        Glide.with(context).load(feeds.get(position).feedPicture).into(holder.binding.image)
